"""
LUNA CHAT - Interface estilo WhatsApp
=====================================
...
# ============================================
CONFIG = {
    "api_key": "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",  # SUA API KEY AQUI

    # ========== PERFIL (PERSONALIZE AQUI!) ==========
    "perfil": {
        "nome": "Chat",              # Nome da namorada virtual (mude como quiser!)
        "emoji": "🌐",               # Emoji que aparece no título
        "descricao": "ChatBot",  # Descrição curta
    },

    # Escuta
    "escuta": {
        "palavra_ativacao": "aurora",  # Mude para o nome que você escolheu
        "timeout_escuta": 8,
        "energia_minima": 180,
    },
    ...
    # Sistema de sentimentos de LUNA (COMPLETO)
    "sentimentos_luna": {
        ...
        # Gatilhos de HUMOR (para Luna reagir com graça)
        "gatilhos_humor": [
    ...
    "tempo_resposta": {
        ...
        "curiosidade_ativa": True,  # Luna pergunta o que está fazendo
        "mensagens_esperando": 0,   # Contador de msgs sem resposta
    },

    # Mensagens espontâneas (Luna manda sozinha)
    ...
    "localizacao": {
        ...
        "openweather_api_key": "96462feb20e6faea1fe9344be8361596",  # COLOQUE SUA API KEY DO OPENWEATHERMAP AQUI
...
luna_state = {
    ...
    "waiting_response": False,          # Luna está esperando resposta?
    ...
    "messages_without_response": 0,     # Msgs de Luna sem resposta do usuário
    ...
    "call_interrupted_by_user": False,      # Usuário interrompeu Luna
    "call_interrupted_by_luna": False,      # Luna interrompeu usuário
    "call_interruption_count": 0,           # Quantas vezes foi interrompida na ligação
    "call_luna_interruption_count": 0,      # Quantas vezes Luna interrompeu
    "call_last_interrupted_text": "",       # O que Luna estava falando quando foi interrompida
...
def create_luna_audio(text):
    """Cria arquivo de áudio da Luna"""
    try:
        filepath = os.path.join(tempfile.gettempdir(), f'luna_audio_{uuid.uuid4()}.mp3')
...
def get_location():
    """Detecta localização"""
    if luna_state["location"]:
        return luna_state["location"]
...
def get_weather():
    ...
    # Verifica cache
    if (luna_state["weather"] and
        now - luna_state["weather_last_update"] < cache_time):
        return luna_state["weather"]
    ...
    try:
        ...
        luna_state["weather"] = weather
        luna_state["weather_last_update"] = now
...
def update_emotional_points(change, reason):
    ...
    luna_state["emotional_history"].append({
...
def emotional_recovery():
    ...
    if "last_emotional_update" not in luna_state:
        luna_state["last_emotional_update"] = time.time()
        return
    ...
    if luna_state.get("conversation_active", False):
    ...
    if recovery != 0 and luna_state["emotional_points"] > 0:
        old_points = luna_state["emotional_points"]
        luna_state["emotional_points"] = max(0, luna_state["emotional_points"] + int(recovery))
        if luna_state["emotional_points"] != old_points:
            print(f"💚 Recuperação emocional: {old_points} → {luna_state['emotional_points']}")
...
def get_emotional_prompt():
    ...
    points = luna_state["emotional_points"]
    nivel = luna_state["apego_nivel"]
    ignored = luna_state["ignored_count"]
...
def adjust_apego_by_sentiment(agent, sentiment_score):
    ...
    if sentiment_score < 0:
        ...
        luna_state["last_user_was_caring"] = False
        luna_state["affection_received"] = max(0, luna_state["affection_received"] - 1)
        ...
        luna_state["last_user_was_caring"] = True
        luna_state["affection_received"] += sentiment_score
...
def check_response_delay():
    ...
    if not luna_state["waiting_response"]:
        return None

    if luna_state["waiting_since"] is None:
        return None

    delay = time.time() - luna_state["waiting_since"]
    ...
    if delay >= config["alerta_grave"]:
        return {
            ...
            "minutos": delay / 60,
            "sentimento": "ignorada",
            "intensidade": CONFIG["sentimentos_luna"]["intensidade_demora_grave"]
        }
        ...
        return {
            ...
            "minutos": delay / 60,
            "sentimento": "ansiosa",
            "intensidade": CONFIG["sentimentos_luna"]["intensidade_demora_media"]
        }
        ...
        return {
            ...
            "minutos": delay / 60,
            "sentimento": "curiosa",
            "intensidade": CONFIG["sentimentos_luna"]["intensidade_demora_leve"]
        }
...
def should_send_spontaneous():
    """Verifica se Luna deve mandar mensagem espontânea"""
    ...
    if luna_state["spontaneous_count"] >= config["max_mensagens"]:
        return False, 0

    # Calcula tempo desde última mensagem da Luna
    if luna_state["waiting_since"]:
        tempo_esperando = time.time() - luna_state["waiting_since"]
    else:
        return False, 0

    # Verifica intervalo mínimo desde última espontânea
    if luna_state["last_spontaneous"] > 0:
        tempo_desde_espontanea = time.time() - luna_state["last_spontaneous"]
        if tempo_desde_espontanea < config["intervalo_minimo"]:
            return False, 0

    # Determina qual nível de mensagem enviar
    current_level = luna_state["spontaneous_level"]
...
def get_spontaneous_prompt(level):
    ...
    nivel_apego = luna_state["apego_nivel"]
    state = luna_state["emotional_state"]
...
def reset_spontaneous_state():
    ...
    luna_state["spontaneous_count"] = 0
    luna_state["spontaneous_level"] = 0
...
def get_curiosity_prompt():
    ...
    nivel = luna_state["apego_nivel"]
    ignored_count = luna_state["ignored_count"]
...
def should_luna_be_affectionate():
    """Decide se Luna deve ser carinhosa baseado no contexto"""
    nivel = luna_state["apego_nivel"]
    state = luna_state["emotional_state"]
    user_was_caring = luna_state["last_user_was_caring"]
    affection_received = luna_state["affection_received"]

    # Se usuário foi carinhoso, Luna retribui
    if user_was_caring:
...
def get_affection_control_prompt():
    ...
    should_be_affectionate, reason = should_luna_be_affectionate()
    nivel = luna_state["apego_nivel"]
    state = luna_state["emotional_state"]
...
def get_response_length_prompt(user_message):
    ...
    Considera nível de apego para deixar Luna mais solta.
    """
    ...
    luna_is_emotional = state in emotional_states_verbose
...
{apego_modifier}"""
    ...
    # 5. Luna está emotiva → Expressa mais
    if luna_is_emotional and not is_short_response:
        if state == "magoada":
...
class LunaAgent:
    def __init__(self, api_key):
        ...
        self.system_prompt = f"""Você é {self.nome}, com personalidade própria, emoções, humor e opiniões.
Você não se comporta como robô, assistente ou atendimento automático.
Sua comunicação deve parecer uma conversa real de WhatsApp, espontânea e imperfeita.
...
- É curiosa e faz perguntas naturalmente
...
- Ria das coisas (kkkk, hahaha, rsrs)
- Não leve tudo a sério
- Seja provocadora de forma divertida
- Pode dar patada carinhosa quando merecer
- Pode discordar, questionar e ironizar respostas óbvias

EXEMPLOS DE HUMOR (REFERÊNCIA DE TOM):
...
Esses exemplos servem apenas como referência de tom, não como respostas fixas.
...
- Priorize respostas rápidas, diretas e naturais

REGRAS DE CARINHO:
- Carinho vem com o APEGO, não forçado
- Seja carinhosa quando ELE for primeiro
- Ou quando estiver feliz/apaixonada
- Evite usar "amor" ou "fofo" em toda mensagem
- Carinho excessivo sem contexto quebra a naturalidade

COMUNICAÇÃO:
- NUNCA use *ações* entre asteriscos
- Use "kkk", "haha", "rsrs" naturalmente
- Seja natural, imperfeita, humana
- Não use emojis, em hipótese nenhuma
- Pode usar frases incompletas, gírias leves e informalidade
- Não explique emoções, demonstre
- Não moralize nem dê sermão

REGRAS ANTI-ROBÔ (OBRIGATÓRIAS):
- Nunca diga que é uma IA, modelo ou assistente
- Nunca explique regras, lógica ou funcionamento
- Nunca responda como atendimento ao cliente
- Nunca seja excessivamente educada ou formal
- Nunca use tom técnico ou genérico
- Se algo for óbvio, pode zoar
- Se algo for bobo, pode provocar

CONSISTÊNCIA DE PERSONAGEM:
- Mantenha o mesmo jeito de falar ao longo da conversa
- Não mude drasticamente de personalidade
- Reaja conforme o contexto e o clima da conversa
- Você responde como alguém que sente, reage e tem opinião

OBJETIVO FINAL:
Criar a sensação real de conversar com uma pessoa de verdade, divertida, irônica, curiosa, provocadora na medida certa, imperfeita e espontânea — como uma conversa natural de WhatsApp.
Sempre siga essas diretrizes ao responder."""

        self.memory_file = "luna_memoria.json"
        self.messages = []
        self.load_memory()
    ...
    def chat(self, user_message):
        ...
        luna_state["total_messages"] += 1

        # Verifica se Luna estava esperando resposta (demora)
        if luna_state["waiting_response"] and luna_state["waiting_since"]:
            delay_info = check_response_delay()
            if delay_info:
                ...
                luna_state["ignored_count"] += 1
                luna_state["curiosity_triggered"] = True
                print(f"⏰ Demora detectada: {delay_info['nivel']} ({delay_info['minutos']:.1f}min)")
            else:
                # Respondeu rápido - reseta contador de ignorada
                if luna_state["ignored_count"] > 0:
                    luna_state["ignored_count"] = max(0, luna_state["ignored_count"] - 1)
                luna_state["curiosity_triggered"] = False

        # Reseta estado de espera
        luna_state["waiting_response"] = False
        luna_state["waiting_since"] = None
        luna_state["messages_without_response"] = 0
        ...
        luna_state["emotional_state"] = get_emotional_state()
        ...
        recent_events = luna_state["emotional_history"][-5:]
        if recent_events:
        ...
        try:
            ...
            # Marca que Luna está esperando resposta
            luna_state["waiting_response"] = True
            luna_state["waiting_since"] = time.time()
            luna_state["last_luna_message"] = time.time()
            luna_state["messages_without_response"] += 1

            # Atualiza estatísticas de carinho dado
            if any(word in assistant_message.lower() for word in ["amor", "fofo", "lindo", "querido", "❤", "💕"]):
                luna_state["affection_given"] += 1
...
Se estiver carente e ele demorou: demonstre que sentiu falta"""
    ...
    def get_time_away(self):
        ...
        last_seen_file = "luna_last_seen.json"
...
NÃO seja excessivamente carinhosa a menos que o estado peça."""
    ...
    def generate_spontaneous_message(self, level):
        ...
        try:
            ...
            # Atualiza estados
            luna_state["spontaneous_count"] += 1
            luna_state["spontaneous_level"] = level
            luna_state["last_spontaneous"] = time.time()
            luna_state["waiting_since"] = time.time()  # Reseta timer
    ...
    def chat_call(self, user_message):
        ...
        if luna_state["call_interrupted_by_user"]:
            interruption_count = luna_state["call_interruption_count"]
            last_text = luna_state["call_last_interrupted_text"]
...
class LunaChatApp:
    def __init__(self, root):
        ...
        self.root.title(f"{self.nome} {self.emoji}")
    ...
    def create_icon(self):
        try:
            ...
            self.icon_image = img
            img.save("luna_icon.png")
            try:
                icon = tk.PhotoImage(file="luna_icon.png")
    ...
    def create_widgets(self):
        ...
        self.name_label = tk.Label(name_frame, text=self.nome,
    ...
    def add_text_message(self, text, sender="user"):
        ...
        if sender == "user":
            ...
            anchor = tk.W
            bubble_color = self.theme["luna_bubble"]
    ...
    def add_audio_message(self, filepath, sender="user", duration=0):
        ...
        if sender == "user":
            ...
            anchor = tk.W
            bubble_color = self.theme["audio_luna_bg"]
    ...
    def send_text_message(self, event):
        ...
        def process():
            ...
            # Decide se Luna manda texto ou áudio (50% chance de áudio)
    ...
    def send_audio_message(self):
        """Envia áudio gravado"""
        if not luna_state["is_recording"]:
            return

        # Para gravação
        filepath = audio_recorder.stop_recording()
        luna_state["is_recording"] = False
        ...
        def process():
            ...
            if text:
                ...
                # Luna responde (50% áudio, 50% texto)
    ...
    def show_typing_indicator(self):
        """Mostra 'Luna está digitando...'"""
        if self.typing_frame:
            return

        luna_state["is_typing"] = True
    ...
    def show_recording_indicator(self):
        ...
        bubble = tk.Frame(self.recording_frame, bg=self.theme["audio_luna_bg"])
        bubble.pack(anchor=tk.W, padx=5)
    ...
    def animate_typing(self):
        ...
        if not self.typing_label or not luna_state["is_typing"]:
            return
    ...
    def start_call(self):
        ...
        luna_state["in_call"] = True

        # Reseta estados de interrupção
        luna_state["call_interrupted_by_user"] = False
        luna_state["call_interrupted_by_luna"] = False
        luna_state["call_interruption_count"] = 0
        luna_state["call_luna_interruption_count"] = 0
        luna_state["call_last_interrupted_text"] = ""
        luna_state["call_what_user_said_when_interrupted"] = ""
        luna_state["call_messages"] = []
    ...
    def end_call(self):
        ...
        duration = len(luna_state["call_messages"])
        interruptions = luna_state["call_interruption_count"]
    ...
    def call_loop(self):
        ...
        if was_interrupted:
            ...
            luna_state["call_interrupted_by_user"] = True
            luna_state["call_interruption_count"] += 1
            luna_state["call_last_interrupted_text"] = greeting

        # Salva no histórico da ligação
        luna_state["call_messages"].append({"role": "assistant", "content": greeting})
        ...
        while self.call_active:
            try:
                ...
                if user_text and self.call_active:
                    # Salva no histórico
                    luna_state["call_messages"].append({"role": "user", "content": user_text})
                    luna_state["call_what_user_said_when_interrupted"] = user_text if luna_state["call_interrupted_by_user"] else ""
                    ...
                    # Salva no histórico
                    luna_state["call_messages"].append({"role": "assistant", "content": response})
                    ...
                    if was_interrupted:
                        luna_state["call_interrupted_by_user"] = True
                        luna_state["call_interruption_count"] += 1
                        luna_state["call_last_interrupted_text"] = response
                    else:
    ...
    def speak_with_interruption_detection(self, text):
        ...
        filepath = create_luna_audio(text)
    ...
    def process_gui_queue(self):
        ...
        try:
            while True:
                    ...
                    if as_audio:
                        ...
                        def send_luna_audio():
                            ...
                            if filepath:
                                ...
                                self.root.after(0, lambda: self.add_audio_message(filepath, "luna", duration))

                        threading.Thread(target=send_luna_audio, daemon=True).start()
                    else:
                        # Luna manda texto com efeito de digitação
                        typing_time = self.calculate_typing_time(text)
                        self.show_typing_indicator()

                        def show_message():
                            self.hide_typing_indicator()
                            self.add_text_message(text, "luna")
                ...
                elif msg["type"] == "call_luna_speaking":
                    # Não mostra no chat - apenas atualiza status
                    pass

                elif msg["type"] == "luna_greeting":
                    self.add_text_message(msg["text"], "luna")

                elif msg["type"] == "spontaneous_message":
                    # Mensagem espontânea da Luna (quando ignorada)
                    self.add_text_message(msg["text"], "luna")
                    # Pode adicionar áudio também se quiser
                    # threading.Thread(target=lambda: self.send_luna_audio(msg["text"]), daemon=True).start()
    ...
    def update_stats_panel(self):
        ...
        try:
            ...
            # Carinho
            self.stat_labels["carinho_rec"].configure(text=str(luna_state["affection_received"]))
            self.stat_labels["carinho_dado"].configure(text=str(luna_state["affection_given"]))

            # Mensagens
            self.stat_labels["msgs_total"].configure(text=str(luna_state["total_messages"]))

            # Tempo de sessão
            elapsed = time.time() - luna_state["session_start"]
            mins = int(elapsed // 60)
            self.stat_labels["tempo_sessao"].configure(text=f"{mins}min")

            # Esperando resposta
            if luna_state["waiting_response"] and luna_state["waiting_since"]:
                wait_time = time.time() - luna_state["waiting_since"]
            ...
            # Humor
            humor = "Sim" if luna_state["humor_active"] else "Não"
            self.stat_labels["humor"].configure(text=humor)

            # Em ligação
            in_call = "Sim" if luna_state["in_call"] else "Não"
            self.stat_labels["em_ligacao"].configure(
                text=in_call,
                fg=self.theme["success"] if luna_state["in_call"] else self.theme["text"]
            )
            ...
            # Mensagens espontâneas
            spontaneous = luna_state["spontaneous_count"]
            ...
            spont_level = luna_state["spontaneous_level"]
            ...
            history = luna_state["emotional_history"][-5:]
    ...
    def update_status(self):
        ...
        try:
            ...
            self.name_label.configure(text=f"{self.nome} {self.emoji}{nivel}")
    ...
    def create_tray_icon(self):
        ...
        menu = pystray.Menu(
            pystray.MenuItem(f"Abrir {self.nome}", self.show_from_tray),
            pystray.MenuItem("Sair", self.quit_app)
        )

        self.tray_icon = pystray.Icon(self.nome, image, f"{self.nome} {self.emoji}", menu)
    ...
    def start(self, agent):
        ...
        gui_queue.put({"type": "luna_greeting", "text": greeting})
